...
export class HotlineQaStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: HotlineQaStackProps) {
    ...
    this.counselorEvaluationsTable = new dynamodb.Table(
      this,
      "CounselorEvaluationsTable",
      {
    ...
    this.counselorProfilesTable = new dynamodb.Table(
      this,
      "CounselorProfilesTable",
      {
    ...
    const startWorkflowFunction = new lambdaNodejs.NodejsFunction(
      this,
      "StartWorkflowFunction",
      {
        ...
        description:
          "Starts the Step Functions workflow for processing call recordings",
      }
    );

    // Create Lambda function to start transcription jobs
    const transcribeFunction = new lambdaNodejs.NodejsFunction(
      this,
      "TranscribeFunction",
      {
        ...
        description:
          "Starts Transcribe Call Analytics jobs for uploaded recordings",
      }
    );

    // Create Lambda function to check transcription job status
    const checkTranscribeStatusFunction = new lambdaNodejs.NodejsFunction(
      this,
      "CheckTranscribeStatusFunction",
      {
        entry: path.join(
          __dirname,
          "../src/functions/check-transcribe-status.ts"
        ),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        timeout: cdk.Duration.seconds(30),
    ...
    const formatFunction = new lambdaNodejs.NodejsFunction(
      this,
      "FormatFunction",
      {
    ...
    const analyzeLLMFunction = new lambdaNodejs.NodejsFunction(
      this,
      "AnalyzeLLMFunction",
      {
    ...
    const aggregateScoresFunction = new lambdaNodejs.NodejsFunction(
      this,
      "AggregateScoresFunction",
      {
        ...
        description:
          "Aggregates scores from LLM analysis and calculates final scores",
      }
    );

    // Create Lambda function for updating counselor records in DynamoDB
    const updateCounselorRecordsFunction = new lambdaNodejs.NodejsFunction(
      this,
      "UpdateCounselorRecordsFunction",
      {
        entry: path.join(
          __dirname,
          "../src/functions/update-counselor-records.ts"
        ),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        timeout: cdk.Duration.seconds(30),
    ...
    const manageCounselorProfilesFunction = new lambdaNodejs.NodejsFunction(
      this,
      "ManageCounselorProfilesFunction",
      {
        entry: path.join(
          __dirname,
          "../src/functions/manage-counselor-profiles.ts"
        ),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        timeout: cdk.Duration.seconds(30),
        ...
        description:
          "Manages counselor profiles via API (GET, PUT, POST operations)",
      }
    );
    ...
    const generatePresignedUrlFunction = new lambdaNodejs.NodejsFunction(
      ...
      "GeneratePresignedUrlFunction",
      {
        entry: path.join(
          __dirname,
          "../src/functions/generate-presigned-url.ts"
        ),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        timeout: cdk.Duration.seconds(30),
    ...
    const getResultsFunction = new lambdaNodejs.NodejsFunction(
      this,
      "GetResultsFunction",
      {
    ...
    const getCounselorDataFunction = new lambdaNodejs.NodejsFunction(
      this,
    ...
    const getAnalysisResultsFunction = new lambdaNodejs.NodejsFunction(
      this,
    ...
    const checkExecutionStatusFunction = new lambdaNodejs.NodejsFunction(
      this,
      "CheckExecutionStatusFunction",
      {
        entry: path.join(
          __dirname,
          "../src/functions/check-execution-status.ts"
        ),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        timeout: cdk.Duration.seconds(30),
        ...
        description:
          "Checks Step Functions execution status for uploaded files",
      }
    );
    ...
    this.counselorEvaluationsTable.grantWriteData(
      updateCounselorRecordsFunction
    );
    this.counselorProfilesTable.grantReadWriteData(
      updateCounselorRecordsFunction
    );
    this.counselorProfilesTable.grantReadWriteData(
      manageCounselorProfilesFunction
    );
    this.counselorEvaluationsTable.grantReadData(
      manageCounselorProfilesFunction
    );
    ...
    const checkTranscribeStatusTask = new tasks.LambdaInvoke(
      this,
    ...
    const formatTranscriptTask = new tasks.LambdaInvoke(
      this,
      "FormatTranscript",
      {
    ...
    const aggregateScoresTask = new tasks.LambdaInvoke(
      this,
      "AggregateScores",
      {
    ...
    const updateCounselorRecordsTask = new tasks.LambdaInvoke(
      this,
      "UpdateCounselorRecords",
      {
    ...
    const definition = startTranscribeTask
      ...
      .next(
        checkJobComplete
          .when(
            sfn.Condition.isNotPresent("$.transcriptKey"),
            waitX.next(checkTranscribeStatusTask)
          )
          .otherwise(
            formatTranscriptTask.next(
              analyzeLLMTask.next(
                aggregateScoresTask.next(updateCounselorRecordsTask)
              )
            )
          )
      );
    ...
    checkExecutionStatusFunction.addEnvironment(
      "STATE_MACHINE_ARN",
      stateMachine.stateMachineArn
    );

    // Grant the check execution status function permission to describe executions
    checkExecutionStatusFunction.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    const getResultsIntegration = new apigateway.LambdaIntegration(
      getResultsFunction
    );
    const getCounselorDataIntegration = new apigateway.LambdaIntegration(
      getCounselorDataFunction
    );
    const getAnalysisResultsIntegration = new apigateway.LambdaIntegration(
      getAnalysisResultsFunction
    );
    const manageCounselorProfilesIntegration = new apigateway.LambdaIntegration(
      manageCounselorProfilesFunction
    );
    const checkExecutionStatusIntegration = new apigateway.LambdaIntegration(
      checkExecutionStatusFunction
    );

    // Add API routes
    api.root
      .addResource("generate-url")
      .addMethod("POST", generateUrlIntegration);
    api.root.addResource("get-results").addMethod("GET", getResultsIntegration);
    api.root
      .addResource("get-data")
      .addMethod("GET", getCounselorDataIntegration);
    api.root
      .addResource("execution-status")
      .addMethod("GET", checkExecutionStatusIntegration);

    // Add analysis route with path parameter
    const analysisResource = api.root.addResource("analysis");
    analysisResource
      .addResource("{fileId}")
      .addMethod("GET", getAnalysisResultsIntegration);
    ...
    const specificProfileResource =
      profilesResource.addResource("{counselorId}");
    specificProfileResource.addMethod(
      "GET",
      manageCounselorProfilesIntegration
    ); // GET /profiles/{counselorId}
    specificProfileResource.addMethod(
      "PUT",
      manageCounselorProfilesIntegration
    ); // PUT /profiles/{counselorId}
    specificProfileResource.addMethod(
      "DELETE",
      manageCounselorProfilesIntegration
    ); // DELETE /profiles/{counselorId}
    ...
    if (deployFrontend) {
      ...
      if (!githubOwner || !githubRepo || !githubTokenSecretName) {
        throw new Error(
          "GitHub parameters (githubOwner, githubRepo, githubTokenSecretName) are required when deployFrontend is true"
        );
      }
    ...
    if (!deployFrontend) {
      const cfnBucket = this.storageBucket.node.defaultChild as s3.CfnBucket;
      cfnBucket.corsConfiguration = undefined;
    }
    ...
    new cdk.CfnOutput(this, "TranscriptsAnalyticsPrefix", {
      ...
      description:
        "S3 prefix for full transcription outputs (automatically created by AWS Transcribe)",
    });
    ...
    new cdk.CfnOutput(this, "CheckExecutionStatusFunctionName", {
      ...
      description:
        "Name of the Lambda function that checks Step Functions execution status",
    });
    ...
    new cdk.CfnOutput(this, "ApiGatewayUrl", {
      value: this.apiUrl,
      description:
        "API Gateway URL - Frontend should use this as base URL for all API calls",
    });
    ...
    if (this.amplifyApp) {
      new cdk.CfnOutput(this, "AmplifyAppUrl", {
        ...
        description:
          "Amplify app URL - Access your deployed frontend application here",
      });
      ...
      new cdk.CfnOutput(this, "AmplifyConsoleUrl", {
        value: `https://console.aws.amazon.com/amplify/home?region=${
          cdk.Stack.of(this).region
        }#/${this.amplifyApp.appId}`,
        description:
          "AWS Amplify Console URL for managing deployments and settings",
      });
    }
  }
}
